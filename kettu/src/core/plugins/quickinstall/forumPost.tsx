...
import { HTTP_REGEX_MULTI, V1VEU_DISCORD_SERVER_ID, V1VEU_PLUGINS_CHANNEL_ID, V1VEU_THEMES_CHANNEL_ID, VD_DISCORD_SERVER_ID, VD_PLUGINS_CHANNEL_ID, VD_PROXY_PREFIX, VD_THEMES_CHANNEL_ID } from "@lib/utils/constants";
...
function useExtractThreadContent(thread: any, _firstMessage = null, actionSheet = false): ([PostType, string]) | void {
    if (thread.guild_id !== V1VEU_DISCORD_SERVER_ID) return;

    // Determine what type of addon this is.
    let postType: PostType;
    if (thread.parent_id === V1VEU_PLUGINS_CHANNEL_ID) {
        postType = "Plugin";
    } else if (thread.parent_id === V1VEU_THEMES_CHANNEL_ID && isThemeSupported()) {
        postType = "Theme";
    } else return;
