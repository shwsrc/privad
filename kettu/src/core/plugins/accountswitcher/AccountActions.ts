...
export const exportAccounts = async (storage: any, showPasswordDialog: any) => {
    ...
    showConfirmationAlert({
      ...
      onConfirm: async () => {
        try {
          ...
          const accountData = Object.values(accounts).map((account: any) => ({
          ...
          showToast(`Exported ${accountData.length} accounts to clipboard`);
        } catch (e: any) {
          addLog('error', 'Failed to export accounts', { error: e.message });
          console.error("Export error:", e);
          showToast("Failed to export accounts");
        }
      }
    });
  });
...
export const importAccounts = async (storage: any, showPasswordDialog: any, importText: string) => {
  const checkPassword = (callback: () => void) => {
    if (storage.settings?.exportPasswordHash) {
      showPasswordDialog({
        type: 'import',
  ...
  checkPassword(() => {
    showConfirmationAlert({
      ...
      content: "Importing accounts is UNSAFE and may compromise your security if the data comes from untrusted sources. Only import data you exported yourself or from sources you completely trust.",
        ...
        try {
          ...
          if (!dataToImport) {
            try {
              ...
              showToast("No data in clipboard or input field");
              return;
            }
          }
          ...
          if (importData.accounts && Array.isArray(importData.accounts)) {
            if (importData.exportPasswordHash) {
              if (!storage.settings?.exportPasswordHash || storage.settings.exportPasswordHash !== importData.exportPasswordHash) {
                ...
                showToast("Import password mismatch or not set locally");
                return;
              }
            }
          ...
          accountsArray.forEach((accountData: any) => {
          ...
          showToast(`Imported ${importedCount} accounts, skipped ${skippedCount} duplicates`);
        } catch (e: any) {
          addLog('error', 'Import failed', { error: e.message });
          console.error("Import error:", e);
          showToast("Failed to import - invalid format or password mismatch");
        }
      }
    });
  });
...
export const setExportPassword = (storage: any, newPassword: string, confirmPassword: string, setNewPassword: (v: string) => void, setConfirmPassword: (v: string) => void) => {
  if (!newPassword.trim()) {
  ...
  showToast("Export password set successfully");
};

export const removeExportPassword = (storage: any, showPasswordDialog: any) => {
  const checkPassword = (callback: () => void) => {
    showPasswordDialog({
      type: 'remove',
  ...
  checkPassword(() => {
    showConfirmationAlert({
      ...
      onConfirm: () => {
        ...
        showToast("Export password removed");
      }
    });
  });
};

export const addAccountWithToken = async (storage: any, newToken: string, setNewToken: (v: string) => void, setIsAdding: (v: boolean) => void) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      showToast("Invalid or expired token");
      setIsAdding(false);
      return;
    }
    ...
    if (storage.accounts[user.id]) {
      ...
      showToast(`Account ${user.username} already saved`);
      setIsAdding(false);
      return;
    }
    ...
    showToast(`Account ${user.username} added!`);
  } catch (e: any) {
  ...
  showConfirmationAlert({
    ...
    onConfirm: async () => {
      try {
        ...
        const fakeToken = "MTExMTExMTExMTExMTExMTEx.GxxxXx.xXxXxXxXxXxXxXxXxXxXxXxXxXxXx";
        ...
        showToast("Force logout successful - you can now login to a different account");
      } catch (e: any) {
        addLog('warn', 'Force logout completed with error', { error: e.message });
        console.error("Force logout error:", e);
        showToast("Force logout completed");
      }
    }
  });
};

export const addAccountWithCredentials = async (storage: any, email: string, password: string, setEmail: (v: string) => void, setPassword: (v: string) => void, setShowAddDialog: (v: boolean) => void, setIsAddingDynamic: (v: boolean) => void) => {
  ...
  try {
    ...
    if (!userResponse.ok) {
      ...
      showToast("Login succeeded but failed to get user info");
      setIsAddingDynamic(false);
      return;
    }
    ...
    showToast(`Account ${user.username} added successfully!`);
  } catch (e: any) {
